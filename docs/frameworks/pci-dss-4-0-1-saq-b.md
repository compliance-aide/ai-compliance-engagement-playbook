# PCI DSS v4.0.1 SAQ B engagement guide
...
Treat SAQ B as a profile-specific validation workflow, not a generic PCI DSS assessment or a conclusion about payment scope. Keep a current record of the accepted payment channels, transaction paths, devices and locations, account-data handling, service providers, connectivity, supporting systems, and material changes. Preserve the completed eligibility analysis and the recipient's instructions with the evidence register. If operations do not match the selected profile, stop the questionnaire workflow and obtain a human scope and profile decision.
