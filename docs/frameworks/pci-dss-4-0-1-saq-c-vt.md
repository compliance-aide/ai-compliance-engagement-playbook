# PCI DSS v4.0.1 SAQ C-VT engagement guide
...
Treat SAQ C-VT as a conditional validation profile with a documented operating boundary. Maintain a current record of the payment channels and transaction process, payment-acceptance technology, accounts and access paths, transaction locations, network and provider dependencies, account-data exposure, and material changes. Capture the eligibility rationale with recipient instructions and do not substitute a questionnaire result for a full scope determination. Escalate new channel, device, connection, provider, process, or evidence conflicts to accountable human owners before continuation.
