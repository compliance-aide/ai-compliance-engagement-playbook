...
**Source and rights snapshot.** Use the guide's cited publisher source (https://le.fbi.gov/cjis-division/cjis-security-policy-resource-center/requirement-companion-document-pdf) and check its current edition or status before use. This plan uses original operational language and does not reproduce protected requirements, assessment questions, mappings, or branded templates. A named human engagement owner confirms applicability and source rights.

**Guide-specific planning input.** Establish CJI boundary and agreements; maintain authorized-user, device, service, vendor, access, training, event, and evidence records; track policy/state-agency changes.
...
### 2. Operating evidence for CJIS Security Policy

- **Request and owner:** Time-bounded design/operating records, asset/configuration, maintenance, test/exercise, supplier, and corrective-action records, selected because this guide focuses on establish cji boundary and agreements; maintain authorized-user, device, service, vendor, access, training, event, and evidence records; track policy/state-agency changes., from engineering, safety, operations, and supplier owners.
