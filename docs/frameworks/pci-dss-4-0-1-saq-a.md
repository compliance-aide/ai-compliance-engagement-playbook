# PCI DSS v4.0.1 SAQ A engagement guide
...
Treat SAQ A as a conditional validation path, not a shortcut or a replacement for PCI DSS scope analysis. Maintain an explicit eligibility record covering payment channels, account-data flows, paper artifacts, merchant webpages, payment-service-provider relationships, systems in the merchant's control, and material changes. Keep the reasoning, supplied provider confirmations, and final confirmation from the acquirer or payment brand separate from the questionnaire evidence. If a fact conflicts with the proposed profile, pause the profile workflow and obtain a human scope decision.
